...
from helper_functions import prepare_multiline_for_latex, text_downloader
from scipy.special import gamma

title_app = (
    "Приложение для расчета усталостной долговечности силовых "
...
def lambda1_compute(
    ...
    return integrate.quad(
        lambda w: w * spectral_density(alpha=alpha, w0=w0, s=s, w=w), 0, np.inf
    )[0]
...
def input_params() -> NoReturn:
    ...
    author = st.sidebar.text_input("Автор работы", value="Иванов И.И.")
...
def order_of_actions():
    ...
    st.markdown(
        "_3. Запустить в рабочей директории проекта компилятор (дважды!)_"
    )
    st.code(
        """
# для сборки каркаса
